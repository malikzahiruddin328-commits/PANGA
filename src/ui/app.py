...
if active_tab == "settings":
    ...
    st.caption(
        "Sourced from ClinicalTrials.gov Phase 3 activity, recent openFDA drug approvals, and "
        "commercial-hiring job postings already in Results (PRD §16a, signals 1-3 of 4). Filtered "
        "to exclude obvious non-companies (universities, hospitals, government), known mega-pharma "
        "majors, and known-acquired companies - but not every remaining entry is a great fit (a "
        "research consortium, an unusual NDA holder, or an unrelated industry whose job title "
        "happened to match can still slip through), so treat \"watching\" as a starting point to "
        "review, not a verified lead. Mark anything wrong as \"disqualified\" below."
    )
