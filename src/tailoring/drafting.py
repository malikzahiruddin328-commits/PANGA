...
def _parse_work_history_date(value: str | None) -> date | None:
    if not value:
        return None
    value = value.strip()
    match = _WORK_HISTORY_MONTH_YEAR_RE.match(value)
    if match:
        month, year = int(match.group(1)), int(match.group(2))
        if 1 <= month <= 12:
            return date(year, month, 1)
        return None
    match = _WORK_HISTORY_YEAR_RE.match(value)
    if match:
        return date(int(match.group(1)), 1, 1)
    return None


def _total_years_of_experience(profile: dict) -> float | None:
    """Real, deterministic candidate-level fact - not a guess or a manually
    set number - derived from the earliest "start" date across the
    master profile's own work_history entries (parses both "MM/YYYY" and
    bare "YYYY" formats, the two real shapes seen in Zahir's actual
    profile data). Feeds ats_score.score_resume_against_keywords's
    years-of-experience/advanced-degree-equivalency credit (2026-08-09,
    Zahir's explicit design) - see that function's own docstring for the
    full policy. Returns None (equivalency never applies) if work_history
    is empty or has no parseable start date, rather than guessing."""
    starts = [d for d in (_parse_work_history_date(e.get("start")) for e in (profile or {}).get("work_history", [])) if d]
    if not starts:
        return None
    return (date.today() - min(starts)).days / 365.25
...
- Named certifications (e.g. ITIL, TOGAF, PMP, an Azure/AWS/GCP certification, CISSP, Security+) the posting asks for but the profile does not show the candidate actually holds (a real match against the profile's own stated certifications/credentials, not just an adjacent skill) split into two very different outcomes depending on the POSTING'S OWN WORDING for that specific certification, not a generic assumption about how important certifications usually are:
  - The posting states it as required/mandatory/must-have ("required," "must have," "mandatory," listed under a "required qualifications" section, etc.) - score this LOW/near 0, the same hard-disqualifier tier as the military-membership and entry-level rules above. A candidate who plainly lacks a certification the posting itself makes non-negotiable is not a fit, regardless of how strong the rest of the match is.
  - The posting states it as preferred/desired/nice-to-have/a plus/bonus - this is NEVER a disqualifier. Do not lower the score just because the certification itself is missing; instead credit the candidate's genuinely relevant industry/domain experience in its place, the same way any other soft "nice to have" gets weighed against real hands-on experience in that area.
  - Judge required-vs-preferred from the posting's actual language for THIS certification specifically (a single posting can require one certification while only preferring another) - don't default to "required" just because a posting mentions a certification with no explicit qualifier, and don't invent a certification requirement the posting never actually names.

Assign a 0-100 fit_score and a one-sentence, specific, plain-language fit_rationale explaining why (not generic filler) via the structured output schema. If a required-but-missing certification is what drove a low score, say so plainly in fit_rationale rather than a vague "not a strong fit"."""
...
Do NOT silently omit a named alternative just because it reads as broad or generic (real gap caught live 2026-08-09: two separate real postings each listed "...Information Technology, Computer Science, Business, or a related field" and the extraction dropped "Business" entirely - not flattened, just missing - while every other field in the exact same list survived). If the posting explicitly names a field or alternative, it is a real, deliberate part of that requirement regardless of how broad it sounds - include it as an atomic any_of member exactly like every other named alternative in the same list, never quietly drop one because it seems too generic to bother with.

Either/or chains can have MORE than two alternatives, and a chain's own alternatives can each carry their own nested field-list (a real gap caught live 2026-08-09 on a real posting, structured like: "Doctorate degree OR Masters degree and 2 years of Computer Science, IT or related field OR Bachelors degree and 4 years of [same field list] OR Associates degree and 8 years of [same field list] OR High school diploma/GED and 10 years of [same field list]"). Do not stop grouping after only two alternatives, and do not flatten a long chain into independent keywords just because it has more than two branches - the same "one satisfied alternative satisfies the whole requirement" logic applies no matter how many tiers there are. Handle a chain like this as TWO separate any_of groups, not one nested structure (this schema only supports a flat list of alternatives per any_of, not an any_of nested inside another): (1) the degree LEVEL across every tier becomes one any_of group, e.g. {"any_of": ["Doctorate degree", "Masters degree", "Bachelors degree", "Associates degree", "High school diploma/GED"]} (the years-of-experience number per tier is dropped per the years-of-experience rule above - it is not a checkable skill), and (2) the field list repeated inside every tier becomes its own separate any_of group, e.g. {"any_of": ["Computer Science", "IT"]}, using the degree-field-lists rule above. A candidate needs to satisfy ONE tier from group (1) and ONE alternative from group (2) - extracting them as two independent groups checks both halves without needing to model which specific tier-to-field pairing the posting intended, which is not a distinction this scorer needs to make.

A term can legitimately need to be extracted BOTH as an any_of alternative AND as its own independent flat required keyword, when the posting genuinely uses it in two distinct roles (real gap caught live 2026-08-09: a posting listed "Supply Chain Management" as an acceptable degree field, AND separately required real, substantive supply chain operations experience/knowledge elsewhere in its own qualifications). Holding a degree in a field is not the same claim as having real hands-on experience in it - satisfying one does not prove the other. When the posting states both, extract the term in both places: once as a member of the degree-field any_of group, and once again as its own ordinary flat required keyword for the substantive experience/knowledge requirement - do not let its appearance in the any_of group suppress extracting it as a real, separate requirement the posting also genuinely asks for."""
...
def rescore_against_cached_keywords(job: dict, app_record: dict, profile: dict) -> dict:
    ...
    ats = score_resume_against_keywords(required, preferred, resume_text, candidate_years_experience=_total_years_of_experience(profile))
...
def _merge_keyword_gap_questions(
    ...
    hit live) ALSO now generates its own real interactive skill_gap
    question, same mechanism as missing_required_keywords below, not just
    a point_value backfill onto the AI's free-form questions. Real gap
    General found live 2026-08-09: preferred terms had no question-
    generation path of their own at all - a missing preferred keyword
    only ever became answerable if the AI's own free-form
    clarifying_questions from the SAME drafting call happened to
    independently think to ask about it; otherwise it sat as passive "How
    to raise it" bullet text forever, with no mechanism to actually
    surface the fact that would close it. Real case: Zahir had genuine
    "clinical-stage organizations" experience worth 6.2 points that was
    never once asked about directly. is_preferred=True marks these so a
    caller can render them with lower-urgency framing than a required gap
    ("would help" vs "the posting requires") - UI refinement's call on
