...
_RESUME_SPEC_COMMON = (
    ...
    "rank-prefix - a higher rank stated before a comma, ahead of the actual "
    "working title (e.g. the master profile's title field literally reads "
    "'Vice President, Head of Applications' - 'Vice President,' is the "
    "prefix, 'Head of Applications' is the working title) - or a "
    "parenthetical seniority-equivalence note after the title (e.g. 'Head "
    "of IT (CIO-equivalent)'). Treat BOTH forms identically: First judge "
    "THIS job posting's own seniority level. If the posting itself is "
    "VP-level or higher, print the title in FULL exactly as stored, prefix "
    "and all (e.g. 'Vice President, Head of Applications', 'Head of IT "
    "(CIO-equivalent)') - they support the case that the candidate has "
    "already operated at that level. If the posting is below VP-level, "
    "print ONLY the working title with the prefix or parenthetical cut "
    "(e.g. 'Head of Applications', 'Head of IT', NOT 'Vice President, Head "
    "of Applications' or 'Head of IT (CIO-equivalent)') - for a role below "
    "that level, keeping the higher-rank qualifier on the printed title "
    "reads as applying beneath your level, and this applies just as much "
    "to a leading 'Vice President,'/'VP,'-style prefix as it does to a "
    "parenthetical. Either way, dropping or keeping this qualifier is not "
    "inventing or embellishing - it is the same underlying role, employer, "
    "dates, and bullet content either way, exactly as given. "
)
