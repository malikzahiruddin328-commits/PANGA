...
| ATS self-score credibility — validate against a real ATS-parsing signal | Medium | None | Not started | 0% | Added 2026-08-01, from `docs/competitive-brief.md` §4a's 4th roadmap item. Not a rating-tier change (fit scoring stays "Strong," can't honestly claim "unique" while JobScan matches it — excluded from the score-target math above) but a real credibility upgrade: benchmark Panga's self-assessed ATS score (LLM-generated, same model that wrote the resume) against a real third-party ATS-parsing signal or JobScan-style keyword-match logic on a sample of real resumes, and disclose the correlation. Removes the one caveat sales conversations currently have to hedge. |
