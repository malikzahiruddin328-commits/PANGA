...
| Browser extension — LinkedIn/Dice job capture | Low | Whether it ships inside the installer is a product decision waiting on Zahir | Built | 100% | Added 2026-08-06/09, found untracked while grooming (no backlog presence despite being fully built). Chrome MV3 extension (`extension/`) captures a job posting's text from LinkedIn/Dice and sends it to Panga via a local HTTP bridge (`src/extension_bridge.py`, fixed port 8765, loopback-only). Polish batch: toolbar badge (green/amber/none per tab reflecting whether a page is ready to send), popup preview (char count + snippet before sending, not a black box), post-send notification with a deep-link back into the Results tab (matched by normalized `posting_url`), and auto-save/auto-score on capture (no manual "Save" click needed, since sending itself is the human confirmation). 550-test suite passing. **One real open item, explicitly a product not technical decision, routed to Zahir and still unanswered:** does `extension/` ship inside the native-packaging installer's file tree, or stay manual-install-only? Packaging investigation confirmed no technical blocker either way (`extension_bridge.py` is stdlib-only, already covered by Phase 2's `hiddenimports`; port 8765 needs no firewall change; Pillow is a one-time icon-build tool, not a runtime dependency) — this is purely "does Zahir want it bundled," nothing left to build to answer it. |
