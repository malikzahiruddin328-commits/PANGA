...
| Licensing / per-user API key handling for a sold product | Not started | 0% | Added 2026-07-31, surfaced while scoping native packaging (row above) once Zahir confirmed the eventual goal is selling this app, not just personal use. Distinct from "Direct LLM API integration" above (that item is about replacing Claude Code orchestration with direct API calls at all — a hard prerequisite for any standalone `.exe`); this item is about what happens once there's more than one paying installation: each customer needs their own way to pay for their own Claude API usage (their own key, or a metered/billed-through-Panga model), plus some form of license/activation so a purchased copy can't be freely copied to unlimited machines. Not designed yet — deliberately deferred until packaging (row above) and the direct-API prerequisite it depends on are further along, since license/billing design depends on how end-user API calls are actually made. |
