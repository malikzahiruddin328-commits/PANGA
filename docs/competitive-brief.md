...
**A note on cross-references, since the two numbering systems in this document look alike but mean different things:** this brief refers to two documents — itself, and Panga's engineering backlog (`docs/job-search-automation-prd.md`). Throughout, **"Section N"** (spelled out) means a section of *this brief*. **"PRD §N"** (with the § symbol) always means a section of the *separate PRD document* — in practice always PRD §13, its single backlog-table section, distinguished further by row name where it matters. The two are never interchangeable and the symbol is reserved for the PRD specifically so they can't be confused at a glance.
...
## 1. What Panga Actually Does Today (verified against the PRD and code, not taken at face value)
...
| Gmail CTA monitoring + auto-draft-reply | Real fulfillment loop (`panga-cta-fulfillment`, every 10 min): detects offer/interview/rejection emails, **drafts** a real reply via Claude, labels the thread — draft only, Zahir sends. | `tailoring/cta_emails.py`, PRD §14 |
| Prospector — proactive target-account ID | Identifies target companies **before jobs post**, using clinical-trial-progress, regulatory-filing, and commercial-hiring signals, plus a KPI/self-scoring loop. **Currently pharma/life-sciences-vertical-specific** — the signal-sourcing stack (openFDA, ClinicalTrials.gov, hiring keywords) is explicitly not yet generalized to other verticals (PRD §13, "Point (4)... intentionally still untouched"). | `prospector/*.py` |
...
| Multi-tenant readiness ("ready for a stranger to install and use") | Absent today — real gap | Adequate (the realistic v1-sellable bar; Strong is a later target once real customers have used it) | This is the single largest item on this roadmap and is already scoped in detail in Section 9 and PRD §13: replace the MCP-connector dependency for Gmail with a direct API integration, finish and test licensing/billing, and validate the fresh-install/onboarding path with someone who isn't Zahir. Do not attempt to jump straight to "Strong" (matching mature SaaS incumbents' polish) — Adequate-and-honestly-scoped is the right first target. |
...
1. **Accept ~17–20% (29→34–35) as the honest near-term target** — built from Section 4a's already-scoped items plus sizing the discovery-breadth candidate above — and revisit the ceiling once Prospector's vertical expansion (a separate, larger initiative, PRD §13) opens genuinely new headroom.
...
4. **The product currently supports this positioning without pharma-specific caveats.** ❌ Not yet true. Prospector — the single strongest piece of evidence for this whole hypothesis — is life-sciences-only today. A generic "senior/executive" positioning needs either (a) Prospector's signal sources generalized to other verticals (partially underway per the PRD's multi-vertical branch, 80% built, but explicitly **not** including Prospector's signal stack), or (b) the initial go-to-market narrowed to "senior/executive life-sciences leaders" specifically, which is a smaller but immediately truthful market to sell into.
...
Grounded in what's already scoped in PRD §13 — this is not a new estimate, it's a synthesis of decisions already made, so the honest gap is smaller than "rebuild from scratch" but larger than "just add billing." This table is also the execution roadmap referenced from the Verdict (Section 11) — the sequence below is what "yes, and here's exactly what's next" means in practice.
...
| Gmail OAuth for non-technical customers | Per-customer Google Cloud project required today — a real barrier for a non-technical buyer | Scoped already (PRD §13) with a real cost (Google CASA assessment) — explicitly deprioritized until volume justifies it, which is a reasonable sequencing call but means early customers face real setup friction |
...
See the resolved data-flow claim in PRD §13 ("Privacy/trust model" row) — Panga's own servers never see resume/profile/application data, only license/billing info; AI drafting goes directly from the user's machine to Anthropic via the user's own account.
