...
**src/extension_bridge.py** (2026-08-08) - local HTTP listener (fixed port 8765, see CLAUDE.md's port convention) for the `extension/` Chrome extension: `/heartbeat` (extension-is-alive ping) and `/capture` (JD text from a LinkedIn/Dice job page). `get_heartbeat_status()`/`get_capture_for_url()` are the UI's read side - `ui/app.py`'s persistent status line and `render_paste_jd_prompt_before_drafting()`'s auto-fill both call in here. `start_server()` runs it on a daemon thread inside the Streamlit process itself - no separate service.

**extension/** (2026-08-08) - the Chrome MV3 extension itself (manifest/background/content/popup), see its own `README.md` for what it does and how to install it (plain-language, written for Zahir not a developer). LinkedIn/Dice only, by design - see its README for the one unverified gap (LinkedIn's JSON-LD extraction path couldn't be tested against a real logged-in page this session).
