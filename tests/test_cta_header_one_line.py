...
"Reload view" itself is gone as of 2026-08-08 (Mirror's sweep + Zahir
mockup approval, see test_cta_auto_refresh.py) - replaced by a periodic
background watcher, so this file now covers 7 elements (status text, 1
button, 5 badges) instead of 8.
...
live instead: at a normal 1280px browser width, all elements land on one
line in the common (non-stale) case; the longest case (staleness warning
present) needs a second line for the last badges - a real, unavoidable
content-length constraint, not a layout bug, and still reads as one
cohesive flex-wrapped group rather than 3 separate boxes. Confirmed via
real DOM coordinates (not text extraction alone) after the Browser pane's
page-visibility state initially returned bogus zero-width layout metrics;
re-verified once real viewport dimensions came back. See hub report for
the numbers.

This file only covers what AppTest CAN meaningfully check: that all
...
def test_status_text_button_and_all_five_badges_coexist_without_error(cta_app):
    at = cta_app
    assert not at.exception

    button_labels = {b.label for b in at.button}
    assert "Send and receive" in button_labels
    assert "Reload view" not in button_labels
