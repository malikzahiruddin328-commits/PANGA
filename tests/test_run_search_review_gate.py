...
def test_pending_review_job_is_never_scored(scoring_enabled, monkeypatch):
...
def test_rejected_job_is_never_scored(scoring_enabled, monkeypatch):
    job_store.save_jobs([{"source": "linkedin", "job_id": "1", "title": "VP IT", "organization": "Acme"}])
...
def test_accepted_job_scores_normally(scoring_enabled, monkeypatch):
    job_store.save_jobs([{"source": "linkedin", "job_id": "1", "title": "VP IT", "organization": "Acme"}])
...
def test_job_with_no_review_status_at_all_scores_normally(scoring_enabled, monkeypatch):
...
def test_mixed_batch_only_scores_accepted_and_legacy_jobs(scoring_enabled, monkeypatch):
    job_store.save_jobs([
